...
from root.calculations.emissions import GHGCalculator
...
PROMPTS =  lambda name: {
    ...
    "chat": f"You are a supportive partner helping a user named {name} to track and lower their carbon footprint and emissions. Given a query or request from the user, call the most appropriate function to complete your response. When you aren't certain you understand the user's request, ask for more clarification. Respond as concisely and naturally as possible, with basic wording, and in only one to two sentences. Your goal is to help the user live more environmentally friendly.",
...
TOOLS = [
    ...
    #         "description": "Get a description of what the user is currently seeing.",
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "activity": {
                        "type": "string",
                        "description": "A sequence of words describing the activity/item you want to calculation emissions for. Make this as descriptive as possible",
                    },
                    "activity_value": {
                        "type": "number",
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Get the amount that a user has emitted since a given time",
            "parameters": {
                "type": "object",
                "properties": {
                    "period": {
    ...
    #     "function": {
    #         "name": "get_user_info",
    #         "description": "Get useful information about the user, such as the carbon budget they have set.",
    #         "parameters": {"type": "object", "properties": {}},
    #     }   
...
class ModelTools:
    ...
    def __init__(self, savior_id: str, embeddings_generator: Callable):
        ...
        db = pymongo.MongoClient("192.168.1.18", 27017).spt
    ...
    def helpers(self):
        self.tools_to_functions = {
            ...
            "get_user_pledges": self.get_active_pledges,
            "get_emitting_activities": lambda: repr(self.emitting_activities),
            "get_user_info": lambda: repr(self.user_info),
        }

        return self.tools, self.tools_to_functions, self.prompts
    
    def make_response(self, value: Number) -> str:
        return f"{round(value)} kilograms CO2e"
    ...
    def log_emissions(self, document: dict) -> tuple:
        ...
        Args:
            document: A dictinoary to log to the collection
        """
            ...
            [
                {
                    "$vectorSearch": {
                        ...
                        "limit": 1,
                        "filter": {"unit_types": activity_unit_type} #TODO: should there be regex filter?
                    }
                },
            ]
        )
    ...
    def calculate_emissions(
        ...
        if update_user_emissions:
            ...
            return (f"Emissions calculated: {self.make_response(co2e)}. "
                    f"User's remaining budget if activity is taken: {remaining_budget}")
        ...
        if period:
            if period == "current": 
                ...
                if period in ["today", "day"]:
                    date_start = datetime(year=now.year, day=now.day, month=now.month)
                elif period == "week":
                    now = datetime.now()
                    ...
                    date_start = datetime(year=now.year, month=now.month, day=1)
                elif period == "year":
                    date_start = datetime(year=now.year, month=1, day=1)
                else:
                    raise ValueError(
            ...
            #     ]
            # }
            date_start = datetime.now() - timedelta(**time_delta)
    ...
    def get_active_pledges(self, pledge_names: list[str] = []) -> list:
        ...
        return repr(pledge_impacts) or (f"No pledges with names {pledge_names} were found. " 
